...
static void *get_filename(GtkWidget *button, gpointer data)
{
	...
	//init dialog
        dialog = gtk_file_chooser_dialog_new("Choose the image wanted", GTK_WINDOW(toplevel), GTK_FILE_CHOOSER_ACTION_OPEN, GTK_STOCK_OK, GTK_RESPONSE_ACCEPT, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, NULL);
...
//	g_signal_connect((G_OBJECT(btn2), "clicked", G_CALLBACK(binarize), image);
